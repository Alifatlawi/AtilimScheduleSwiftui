...
class DataClassifier: ObservableObject{
    ...
    var sharedData: SharedData = SharedData.shared
    func getData() {
        isLoading = true
        
        guard let facultyLink = sharedData.facultyLink else {
            print("Faculty link is not set.")
            print(sharedData.facultyLink ?? "")
            return
        }
        
        guard let urlComponents = URLComponents(string: facultyLink),
              let tdNum = urlComponents.queryItems?.first(where: { $0.name == "num" })?.value,
              let domain = urlComponents.host else {
            print("Invalid faculty link format")
            return
        }
        
        let prefixedDomain = domain.hasPrefix("https://") ? domain : "https://\(domain)"
        ...
        let headers = [
            "authority": domain,
            "origin": prefixedDomain,
            "referer": prefixedDomain,
            "cookie" : "PHPSESSID=3c6a20fde6e9fdae72ad787719a257b1"
        ]
        
        var request = URLRequest(url: URL(string: "\(prefixedDomain)/timetable/server/regulartt.js?__func=regularttGetData")!)
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                if self.sharedData.facultyName == "engineering" {
                    let DataModel = try decoder.decode(Welcome.self, from: data)
                    DispatchQueue.main.async {
                        self.processData(welcomeData: DataModel)
                        self.isLoading = false
                    }} else if self.sharedData.facultyName == "business" {
                        let DataModel = try decoder.decode(BusinessModel.Welcome.self, from: data)
                        DispatchQueue.main.async {
                            self.processDataB(welcomeData: DataModel)
                            self.isLoading = false
                        }
                    } else if self.sharedData.facultyName == "artsAndSciences" {
                        let DataModel = try decoder.decode(BusinessModel.Welcome.self, from: data)
                        DispatchQueue.main.async {
                            self.processDataB(welcomeData: DataModel)
                            self.isLoading = false
                ...
                else if self.sharedData.facultyName == "fineArts" {
                    let DataModel = try decoder.decode(BusinessModel.Welcome.self, from: data)
                    DispatchQueue.main.async {
                        self.processDataB(welcomeData: DataModel)
                        self.isLoading = false
                ...
                else if self.sharedData.facultyName == "healthSciences" {
                    let DataModel = try decoder.decode(BusinessModel.Welcome.self, from: data)
                    DispatchQueue.main.async {
                        self.processDataB(welcomeData: DataModel)
                        self.isLoading = false
                ...
                else if self.sharedData.facultyName == "civilAviation" {
                    let DataModel = try decoder.decode(AvmModel.Welcome.self, from: data)
                    DispatchQueue.main.async {
                        self.processDataAvm(welcomeData: DataModel)
                        self.isLoading = false
                    }
                }
    ...
    func processDataB(welcomeData: BusinessModel.Welcome) {
        guard let lessonsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "lessons" }),
              let classroomsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "classrooms" }),
              let subjectsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "subjects" }),
              let periodsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "periods" }),
    ...
    func processDataAvm(welcomeData: AvmModel.Welcome) {
        guard let lessonsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "lessons" }),
              let classroomsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "classrooms" }),
              let subjectsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "subjects" }),
              let periodsTable = welcomeData.r.dbiAccessorRes.tables.first(where: { $0.id == "periods" }),
